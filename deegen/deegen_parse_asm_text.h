...
struct X64AsmLine
{
    ...
    {
        if (!IsInstruction()) { return false; }
        bool res = x_targetX64 ? (GetWord(0) == "jmp") : (GetWord(0) == "b");
    ...
    void FlipConditionalJumpCondition()
    {
        ...
        else
        {
            ...
            ReleaseAssert(false && "Unexpected conditional branch");
        }
    }
    ...
    static X64AsmLine WARN_UNUSED Create(std::string label)
    {
        if (x_targetX64)
        {
            return X64AsmLine::Parse("\tjmp\t" + label);
        }
        else
        {
            return X64AsmLine::Parse("\tb\t" + label);
        }
    }

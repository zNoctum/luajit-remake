...
BaselineCodeBlock* NO_INLINE deegen_baseline_jit_do_codegen(CodeBlock* cb)
{
    ...
    //
    constexpr size_t x_maxBytesCodegenFnMayOverwrite = x_targetX64 ? 7 : 4;
    ...
    //
    {
        ...
        {
            static_assert(x_maxBytesCodegenFnMayOverwrite >= (x_targetX64 ? 2 : 4) /*length of ud2/udf instruction*/);
            static_assert(x_targetX64 ? true : x_maxBytesCodegenFnMayOverwrite % 4 == 0);
            if (x_targetX64)
            {
                // ud2: 0x0f, 0x0b
                //
